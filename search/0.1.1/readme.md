...
#import "@rookery/search:0.1.1": search-bar
#show: rookery
...
`idea-href()` plus a `fetch`, which is what `#search-modal`'s own preview pane
...
```typst
#import "@rookery/search:0.1.1": search-ideas
#context {
...
itself and does not import `#idea-tag-names`: rookery owns the rule — the same one
`#window(tagged: …)` applies — and this is a pass-through. One rule written twice
...
```typst
#import "@rookery/core:0.1.1": ideas, tag-index
#import "@rookery/search:0.1.1": panel
...
#context panel(
  rows: ideas(tagged: "submission", index: INDEX),
  facets: ("sort", "state"),
  sort: "deadline",
  visible: 5,
  noun: "submissions",
...
```typst
#import "@rookery/search:0.1.1": filter-panel
...
```typst
#import "@rookery/timeline:0.1.1": upcoming-rows
#import "@rookery/search:0.1.1": filter-panel
...
- Clicking a tag chip in an entry's hat toggles the same filter as its pill —
  pressing it again releases it, exactly like the pill. Only a chip whose tag
  has a pill in this container reacts; any other chip stays inert.
...
ln -s "$PWD" ~/.cache/typst/packages/rookery/search/0.1.1
```
