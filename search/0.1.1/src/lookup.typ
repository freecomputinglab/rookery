...
#import "@rookery/core:0.1.1": ideas
#import "base.typ": *
#import "rank.typ": *
...
//
// `tags:`/`match:` are the same pair rookery's own `ideas()` takes (as
// `tagged:`/`match:`), passed straight through — `tags` is `none` for the
// whole rookery, one string, or an array of strings;
// `match` is "any" or "all". Nothing is re-filtered here and `idea-tag-names` is
