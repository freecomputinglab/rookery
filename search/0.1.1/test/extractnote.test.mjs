...
test("extractNote: window wrapper preserves margin notes, footnotes, and references unchanged", () => {
  const body = `<p>One<sup class="idea-fn-ref" id="fnref-1-1" data-rookery="fn-ref"><a href="#fn-1-1">1</a></sup><span class="idea-sidenote" id="fn-1-1" data-rookery="sidenote"><span class="idea-sidenote-number" data-rookery="sidenote-number">1</span> First note.</span></p><div class="idea-footnotes" data-rookery="footnotes"><h4 class="idea-footnotes-title" data-rookery="footnotes-title">Footnotes</h4><ol class="idea-footnote-list" data-rookery="footnote-list"><li class="idea-footnote" id="fn-1-1" data-rookery="footnote"><a class="idea-fn-backlink" href="#fnref-1-1" data-rookery="fn-backlink">^</a> First note.</li></ol></div><div class="idea-references" data-rookery="references">References here</div>`;
  const doc = parse(`<!doctype html><body>
    <div class="idea-head"><h1 class="idea">Title</h1></div>
    ${body}
    <footer class="idea-footer">f</footer>
  ...
  const box = extractNote(doc, "https://example.org/x.html");
  const inner = box.querySelector(".idea-window-body");
  assert.equal(inner.outerHTML.includes('data-rookery="window-body"'), true);
  assert.equal(inner.innerHTML, body);
  assert.notEqual(inner.querySelector('[data-rookery="sidenote"]'), null);
  assert.notEqual(inner.querySelector('[data-rookery="footnotes"]'), null);
  assert.notEqual(inner.querySelector('[data-rookery="references"]'), null);
});
