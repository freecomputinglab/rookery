...
- type:feature
deps:
- blocked-by:rk-extract-a-shared-todo-table-builder-9971e639
- blocked-by:rookery-priority-tests-hax
closed: false
---
`@rookery/todos` has one panel, `#filter-panel`, and it lists EVERYTHING open, ordered by date. That is the right shape for a worklist and the wrong shape for the question a person asks at the start of a day: what is on for today, and what is the most important thing outstanding. Answering it today means reading a list of several hundred rows.
...
It is a SELECTION and nothing else. The projection, the pills, the date-cell ramp and the row rendering all come from `_todo-table`, the shared builder added by bird rk-extract-a-shared-todo-table-builder-9971e639 ("Extract a shared todo table builder"), which lives in `/home/lox/code/_fcl/rookery/todos/0.1.0/src/table.typ` and takes `rows:` (the selected rows), `corpus:` (every todo, so the dependency graph can resolve a closed blocker), `today:`, and the panel knobs `facets`, `tag-filter`, `pill-rows`, `when`, `order`, `countdown`, `overdue`, `undated-priority`, `visible`, `placeholder`, `noun`, `empty`, `haystack`, `render`.
...
1. Create `src/today.typ` with a file header in this repo's comment style — present tense, describing what the file is, one header and no interior banners, no issue ids (see `/home/lox/code/_fcl/rookery/CLAUDE.md`). What it says: the day view is a SELECTION over the shared todo table, and the two questions it unions — what is dated for today, and what is most important — are different questions that a person starting a day asks together.
...
4. Define `#today-panel`. Document every parameter in the style `#filter-panel` uses (`src/panel.typ:146-270`) — a paragraph per argument saying what it does and why the default is the default, not a one-line gloss:
   ...
   #let today-panel(
     ...
     tag-filter: none,
     pill-rows: <the same default literal as panel.typ:204-208>,
     when: none,
     order: "soonest",
     countdown: true,
   ...
   - `overdue: true` — whether a deadline already behind you is listed. `true` lists it, and `_todo-table` paints its date cell the overdue band. `false` drops it. On by default: a day view that silently hides what is late is worse than no day view.
   ...
   `visible: none` rather than `#filter-panel`'s `8`: a day view is meant to be read whole, and a scroll box over a list of six rows hides the sixth behind a gesture nothing advertises.

5. Add `#import "today.typ": *` to `src/lib.typ`, immediately after the `#import "panel.typ": *` at line 29, with a short comment in that file's own idiom saying it comes after `table.typ`'s consumer for the same dependency-order reason the rest of that list is ordered. Do not disturb the existing order — `skin.typ` must stay LAST, because it is what shadows `window`.
...
- Do not change `src/panel.typ`, `src/table.typ`, `src/views.typ`, `src/graph.typ` or `src/todos.css`. `#today-panel` reuses the table's existing classes (`todo-when-overdue`, the countdown bands, the priority label cell) and needs no new CSS.
- Do not add a heading, a count line, or any chrome of its own. `_todo-table` renders the whole widget.
