...
#import "@rookery/todos:0.1.1": todo, todos-ready, todo-graph-view
#show: rookery
...
```typst
#import "@rookery/todos:0.1.1": idea, rookery, todo, window
```
...
`#window(tagged:)`/`#ideas(tagged:)` *and* by `@rookery/search`'s tag query
language.
...
presence-filterable — `#window(tagged: "todo-deps")` finds every todo with
dependencies.
...
- **Dates come from [`@rookery/timeline`](../../timeline/0.1.1)** —
...
no `#window(tagged: ..)` selection can express them. Only code that has already
...
A TAG-SELECTED window is still the exception: `#window(tagged: ..)` produces no
...
Use [`@rookery/search`](../../search/0.1.1). This filters *the todos on
...
[`union:`](../../search/0.1.1/readme.md#union--groups-that-answer-one-question), and
...
```typst
#import "@rookery/slipshow:0.1.1": slipshow
#import "@rookery/todos:0.1.1": todo-slipshow
...
**A project using `#todo-slipshow` must import `@rookery/slipshow:0.1.1` in
