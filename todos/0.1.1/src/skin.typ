...
//
//   #import "@rookery/todos:0.1.1": idea, todo, window, rookery
...
#import "@rookery/timeline:0.1.1" as _tl
#import "@rookery/core:0.1.1": idea-tag-names
...
#let window(..args, closed: false) = context {
  ...
  // `idea-tag-names` takes the same name forms `#window` itself accepts — a bare name, a
  ...
  // `idea-tag-names` returns tag NAMES, not the tag dictionary — MEASURED, passing its
  ...
  if CLOSED-KEY in idea-tag-names(pos.at(0)) { return }
  _tl.window(..args)
}
