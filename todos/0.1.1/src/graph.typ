...
#import "@rookery/core:0.1.1": ideas, tag-data
#import "@rookery/timeline:0.1.1": is-scheduled-now, scheduled-of
#import "target.typ": *
#import "tags.typ": *
...
// tag store; joining them on `id` costs nothing. Reaching for `idea-tag-names` or
// `idea-tag-value` per row instead would pay one registry resolution PER NOTE — a
// cost rookery's own `data.typ` documents explicitly against `idea-tag-names`.
//
...
// `idea-tag-names`, where an unknown id answers emptily rather than panicking: a
