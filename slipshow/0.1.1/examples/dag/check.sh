...
# Modelled on `search/0.1.1/demo/rheo/check.sh` and `../ordering/check.sh`:
...
# 7. The status rail: `todo-slip-keys`'s `class:` function (`todos/0.1.1/
...
# 8. A DANGLING DEP DOES NOT BLOCK (`is-blocked`, `todos/0.1.1/src/
...
#     (`data-slip-edges`, `slipshow/0.1.1/src/slipshow.typ`'s header).
#
