...
#import "@rookery/slipshow:0.1.1": slip, slipshow
#show: rookery
...
```typst
#import "@rookery/core:0.1.1": idea as _idea
#import "@rookery/slipshow:0.1.1": slip as _slip
...
`#ideas(tagged:, match:)`, along with `match:`, `"any"` by default), OR a
...
```typst
#import "@rookery/search:0.1.1": parse-tag-query, eval-tag-query
...
Flat keys are filterable by core's own `#ideas(tagged:)`/`#window(tagged:)` and
