...
#import "@rookery/core:0.1.1": idea-row, ideas
...
// ---- #timeline-upcoming --------------------------------------------------------------
//
// `tags:`/`match:`/`filter:` are rookery's OWN selection vocabulary — the same
// pair `ideas()` takes as `tagged:`/`match:` — passed straight through: `ideas()`
// asserts on the first two, and `filter:` is a
...
#let timeline-upcoming-rows(
  ...
  let rows = ideas(tagged: tags, match: match, values: true)
