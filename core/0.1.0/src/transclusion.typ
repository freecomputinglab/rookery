...
#let _window-content(id, rec, shown, folded, display, foldable: true, reserve-title: true, windows-claim: false, rest: none) = {
  ...
  if _target() == "html" or _target() == "epub" {
    ...
    // therefore inert here — the same inertness it already has on the `unfurl: 0`
    ...
    html.elem(
      ...
      // A truncated window nests a SECOND `<details>` inside its body: the
      // shown blocks sit in that inner `<summary>`, so clicking anywhere in
      // them — not only the ellipsis — unfurls the rest in place.
      //
      // The summary carries PLAIN `shown`, not `_footnoted(shown)`, and the
      // body footnotes `shown + rest` together in ONE call rather than
      // `shown` and `rest` each through their own: `_footnoted` numbers from
      // 1 within whatever it is given and closes its own footnote list at
      // the end of it, so two separate calls over one body's two halves
      // restart the count at the split and print two footnote lists instead
      // of one — MEASURED with a two-footnote fixture, half in each half.
      // A footnote inside `shown` therefore renders twice, once as a bare
      // reference in the summary (nothing claims its `<rkfn>` there, so
      // Typst's own document-wide footnote mechanism picks it up) and once
      // correctly inside the numbered list below — the cost of one
      // consistent numbering rather than two inconsistent ones. Citations
      // share the same reasoning: one `_refs-block` over `shown + rest`.
      //
      // THE TWO HALVES ARE REJOINED WITH A `parbreak`, for the reason
      // `_truncate-split` joins the blocks WITHIN each half with one: the
      // split dropped the separator that stood between the last shown block
      // and the first hidden one, and Typst's HTML export decides paragraphs
      // by the `parbreak`s it finds. MEASURED on `rookery.ohrg.org`, with a
      // two-paragraph note behind `limit: 1`: the opened body came out as
      // `…like any other in the rookery.A second paragraph…`, one run with
      // no break and no `<p>` wrappers at all.
        ...
        summary + html.elem("div", attrs: (class: _c("window-body"), data-rookery: "window-body"),
          if rest == none {
            ...
            html.elem("details", attrs: (class: _c("window-more"), data-rookery: "window-more"),
              html.elem(
                "summary",
                attrs: (class: _c("window-more-summary"), data-rookery: "window-more-summary"),
                shown
...
// document-wide default (`#show: rookery.with(window-unfurl: n)`), which is what
// lets a `#window` unfurl by the document's setting without naming a number.
//
// `.marrow.typ` passes an EXPLICIT depth instead, `window-unfurl + 1`: a minted
