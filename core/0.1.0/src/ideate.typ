...
#let ideate(body, separator: none, title: none, name: auto, tags: (), display: (:), display-frame: false, display-id: false, ..args) = context {
  ...
  //
  // Returns `bare` unchanged: `mint(..)` takes it positionally, going through
  // `#idea`'s NAMED path, so this is a pin like any other and must never
  // move — two sections resolving to the same bare name collide there
  // exactly as two hand-written `#idea(<name>)` calls would, surfacing as
  // the registry's duplicate-note-id panic (idea.typ), which names both
  // origins and points at retitling or pinning one of them explicitly.
  let _free-id(bare) = bare
  for group in groups {
