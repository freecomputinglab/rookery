...
#context {
  ...
  // THE DEPTH A MINTED PAGE RENDERS ITS NOTE AT, and it is `window-unfurl` PLUS
  // ONE rather than `window-unfurl` itself.
  //
  ...
  // `window-unfurl` governs is windows nested inside a transcluded body, and on
  ...
  // TOP-LEVEL window has", which is `window-unfurl` itself whatever number that
  ...
  // At `window-unfurl: 0` it is flattened at 1, so a window on a minted page
  ...
  // This page's footnote AND citation modes, read the same way: a minted
  // page never calls `rookery()`, so it cannot see the `footnotes:`/
  // `citations:` parameters template.typ's own emission reads directly, and
  // reads `_footnote-mode.final()`/`_citation-mode.final()` instead — the
  // one remaining state read on the html mode marker's value (see
  // template.typ's emission of the same element for why every other read of
  // either mode is gone).
  let footnote-mode = _footnote-mode.final()
  let citation-mode = _citation-mode.final()
  ...
  for (id, rec) in registry.pairs() {
    ...
    let page = [
      ...
      // state instead of a parameter.
      #html.elem(
        "div",
        attrs: (
          data-rookery: "mode",
          data-rookery-footnotes: footnote-mode,
          data-rookery-citations: citation-mode,
          hidden: "hidden",
        ),
      )
      ...
      #{
        ...
        //
        // BUILT FROM `_page-href` PLUS `id` AS A FRAGMENT, not from
        // `link(label(id), ..)` — the note's own anchor on the origin page
        // carries `id` as its literal HTML `id` attribute (see `#idea`'s own
        ...
        let context-part = if origin == none or not use-context { [] } else {
          let context-href = _page-href(origin, here: page-at.handle)
          let context-dest = if context-href != none { context-href + "#" + id } else { label(id) }
          section(_c("context"), "context", [Context],
            page-list((link(context-dest, _handle-title(origin, mode: page-titles)),)))
        }

        let backlinks-part = if not use-backlinks or (back.len() == 0 and back-pages.len() == 0) { [] } else {
          // FOLDED and `unfurl: 1`, always: a backlink list is an index of what
          // points here, and a reader following one wants to see which notes
          // those are before reading any of them in full. `unfurl` is pinned
          // for the same reason `folded` is, and NOT left at `auto` — a
          // document that set `window-unfurl` to unfurl its prose would
          // otherwise unfurl every entry of every index too, several levels
          // into notes the reader has not chosen yet. MEASURED on a
          // `window-unfurl: 2` project: `ideas/leaf.html`'s Backlinks showed
          ...
          //
          // `unfurl: 1`, NOT `0`, since the rebasing of the scale (see
          ...
          let note-rows = if back.len() == 0 { [] } else {
            window(back.map(b => b.trim(_pfx(), at: start)), folded: true, unfurl: 1)
          }
          ...
          let page-rows = if back-pages.len() == 0 { [] } else {
            page-list(back-pages.map(handle => context {
              let href = _page-href(handle, here: page-at.handle)
  ...
  if _index-page.final() and registry.len() > 0 {
    // NOT `.filter(e => e.href != none)` any more, even though `e.href` (an
    // `ideas()` row's `_note-href`) is exactly the UNMINTED GUARD that reads
    // as — `_note-href` returns `none` under plain `typst compile` and under
    // the combined-PDF target, where no per-note page exists to link to.
    // This whole block is unreachable under either: `.marrow.typ` is spliced
    // in only where rheo mints HTML/EPUB pages (this file's own banner), so
    // by the time `_index-page.final()` gates us in, every registry entry
    // already gets a page and the guard never once triggers. Evaluating
    // `e.href` anyway costs a `state("rheo-handle")` read per row for
    // nothing — the exact stale read the banner above `rows` warns about,
    // just for a value this filter throws away, and one Typst reports as
    // "did not converge" the moment a titleless note anywhere in the spine
    // needs `context` to name its own id. Dropping the filter removes that
    // read entirely instead of merely tolerating its staleness.
    let rows = ideas()
    let page = [
