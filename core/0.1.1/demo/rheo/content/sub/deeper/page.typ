...
#import "@rookery/core:0.1.1": window, ideas-outline, idea-body
#show: demo
...
// unfurl: 0 — a LINK to the note's page, no body at all.
#window("w-outer", unfurl: 0)

// unfurl: 1 (auto/default) — renders Outer once, collapsing its own nested
// #window("w-inner") to a bare permalink.
#window("w-outer", unfurl: 1)

// unfurl: 2 — unfurls the nested #window("w-inner") as a real window too.
#window("w-outer", unfurl: 2)
...
#window("self-loop")
#window("cycle-a")
#window("cycle-a", unfurl: 3)

== A tag window written inside a tagged note

A regression fixture: a `#window(tagged: ..)` written inside a note carrying
...
// `scope: "page"` — this PAGE's own ideas only, per `state("rheo-handle")`.
// Under rheo, unlike under a native compile, this genuinely differs from the
// default form below: every other note on this page belongs to this
// vertebra, but so does every note elsewhere in the rookery, and only the
// default `scope` lists those too.
#ideas-outline(title: [This page's ideas], scope: "page")

// The default `scope` — every note in the rookery, not only this page's.
#ideas-outline(title: [Whole rookery])

// `depth: 1` — caps to the top level only, dropping "Outline child".
#ideas-outline(title: [Depth-capped], depth: 1)

// `title: none` — omits the heading entirely; the list still renders.
#ideas-outline(title: none)
