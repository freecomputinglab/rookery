...
#import "@rookery/core:0.1.1": footnote, ideas-outline, window
...
#note("plain-note", title: [Plain note], created: datetime(year: 2026, month: 5, day: 2))[
  ...
  match (`notes.position(n => n == it)`) gives both the SAME number, always
  the first one's — this pair is the regression fixture for numbering by
  position instead. Transcluded into `sub:page` as a `#window`
  (`content/sub/page.typ`), so `check.sh` can assert the same two numbers on
  both this note's own minted page and the window's page.
