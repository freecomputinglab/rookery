...
// `idea as core-idea`: this page's own `idea` (above) is the project's wrapped
// constructor (`_idea.with(exclude-tags: ..)` in `lib.typ`), a different
// function value from the package's own — `#outline(target: ..)` dispatches
// by identity against the LATTER, so the plain import is what the calls below
// need.
#import "@rookery/core:0.1.1": hyperlink, idea as core-idea, ideas-outline, outline, window
#show ref: hyperlink
#show: demo
...
// under `tagged: "phd"` it is pruned and the child re-based to the top level
...
// it. Both list the whole rookery (the default `scope`), since this
// rookery's other notes live on other vertebrae.
#ideas-outline(title: [Everything])
#ideas-outline(title: [Tagged phd], tagged: "phd")

// The second outline again, spelled through the `target:` front door:
// `#outline(target: core-idea, ..)` forwards its other arguments straight to
// `#ideas-outline` above and renders the same tree.
#outline(target: core-idea, tagged: "phd")
